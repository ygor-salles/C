...
struct Indices
{
    int vendas, imob, atrl[10], disp[10], clientes;
};
...
void encerramento_exercicio (int qtd, Registro atual[], Registro antigo[], Registro *soma_atual, Registro *soma_antigo, int cod_vendas);
...
void relatorio_indices(int qtd, Registro atual[], Registro antigo[], Registro soma_atual, Registro soma_antigo, Indices codigo, int m, int k);
...
void menu_relatorios (int qtd, Registro atual[], Registro antigo[], Lista *li)
{
    ...
    encerramento_exercicio (qtd, atual, antigo, &soma_atual, &soma_antigo, codigo.vendas);
    limpa_tela();
    ...
    if (op == 'S' || op=='s'){
        ...
        printf("\nInforme o(s) código(s) de disponívei(s), tecle 0 para concluir ... \n");
        do{
            scanf("%d",&codigo.disp[k]);
            k++;
        } while(codigo.disp[k] != 0);
        printf("\nInforme o(s) código(s) de Ativo Realizavel Longo Prazo, tecle 0 para concluir ... \n");
        do{
            scanf("%d",&codigo.atrl[m]);
            m++;
        } while(codigo.atrl[m] != 0);
     ...
     do{
        ...
        switch(opcao){
            case 0: encerramento_exercicio (qtd, atual, antigo, &soma_atual, &soma_antigo, codigo.vendas); break;
            ...
            case 3:
                if (op=='S' || op=='s') relatorio_indices(qtd, atual, antigo, soma_atual, soma_antigo, codigo, m, k);
...
void encerramento_exercicio (int qtd, Registro atual[], Registro antigo[], Registro *soma_atual, Registro *soma_antigo, int cod_vendas)
{
    ...
    if (op==2){
        for (i=0; i<2; i++){
            ...
            printf("\nInforme total do Ativo Circulante: "); scanf("%f",&ac[i]);
            printf("Informe total do Ativo Não Circulante: "); scanf("%f",&anc[i]);
            printf("Informe total do Passivo Circulante: "); scanf("%f",&pc[i]);
            printf("Informe total do Passivo Não Circulante: "); scanf("%f",&pnc[i]);
            printf("Informe total do Patrimônio Líquido: "); scanf("%f",&pl[i]);
            printf("Informe total do DRE: "); scanf("%f",&dre[i]);
    ...
    if (op==2) { soma_atual->ac = ac[0]; soma_antigo->ac = ac[1]; }
    ...
    if (op==2) { soma_atual->anc = anc[0]; soma_antigo->anc = anc[1]; }
    ...
    if (op==2) { soma_atual->pc = pc[0]; soma_antigo->pc = pc[1]; }
    ...
    if (op==2) { soma_atual->pnc = pnc[0]; soma_antigo->pnc = pnc[1]; }
    ...
    if (op==2) { soma_atual->pl = pl[0]; soma_antigo->pl = pl[1]; }
    ...
    if (op==2) { soma_atual->dre = dre[0]; soma_antigo->dre = dre[1]; }
...
{
    int i, j;
    float imob_atual, atrl_atual=0, disp_atual=0, client_atual, vendas_atual;
    float imob_antigo, atrl_antigo=0, disp_antigo=0, client_antigo, vendas_antigo;
    ...
    for (i=0; i<qtd; i++){
        ...
        for (j=0; j<m; j++)
            if (codigo.atrl[j]== atual[i].cod){
                atrl_atual = atrl_atual + atual[i].valor;
                atrl_antigo = atrl_antigo + antigo[i].valor;
            }
        for (j=0; j<k; j++)
            if (codigo.disp[j] == atual[i].cod){
                disp_atual = disp_atual + atual[i].valor;
                disp_antigo = disp_antigo + antigo[i].valor;
            }
    ...
    printf("\nLiquidez Seca                             %.2f    %.2f  ", (disp_atual/client_atual)/soma_atual.pc, (disp_antigo/client_antigo)/soma_antigo.pc );
