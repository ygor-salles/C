  ...
  if(*raiz == NULL){
    *raiz = novo;
    ...
    while(atual != NULL){
      ...
      // verifica se o chave novo já existe na árvore
...
int remover_arvore_binaria(Pno *raiz, int chave)
{
  ...
  while(atual != NULL){
    ...
    // verifica se o chave buscado foi encontrado
    if(chave == atual->chave){
    ...
    // verifica em qual subarvore o chave procurado pode estar
