...
void menu(deque *d, deque *e) {                           //função que mostrará o indíce das outras funções
    ...
    int j,compara=0; //variaveis
    char pal[4];
    printf("            Checar palíndromo usando TAD DEQUE\n\n");
    
    //abre o arquivo a ser manipulado
    ...
    //pede ao usuario digitar o número a ser verificado
    printf("\n\nDigite o número a se verificar(3 dígitos): ");
    //lê o número e insere no arquivo
    for(j=0;j<4; j++) {
        setbuf(stdin,NULL);
        scanf("%c",&pal[j]);
        fprintf(arq,"%c",pal[j]);
    ...
    //adicionar aos dois deques
    for(j=0;j<3;j++){
        insere_inicio(d,pal[j]);
        insere_inicio(e,pal[j]);
    }
    //se eles forem palíndromos, o consulta inicio(d) e o consulta final(e) têm que ser iguais
    //se for, incrementa a variável compara e remove o inicio e o final dos deques, respectivamente
    for(j=0;j<3;j++){
        if(consulta_inicio(d)==consulta_final(e)){
            compara++;                   
            remove_inicio(d);
            remove_final(e);
        }}
        //como são 3 algarismos, se houver 3 comparações iguais, ele é um palíndromo
        if(compara == 3)
