...
/* A lógica está certa com apenas um Nome e não com sobrenomes */

//Questão A
typedef struct funcionario
{
    int dias_falta, dep, nascimento_dep[10];
    char transporte, refeicao, nome[51], parentesco[10][51];
    float sb, hr_ext;
} tfuncionario;
...
void imprime_dados (tfuncionario list[], int n)
{
    ...
    for (i=0; i<n; i++){
        ...
        if (list[i].dep>0){
            for (j=0; j<list[i].dep; j++){
                if (list[i].nascimento_dep[j] > 2004){
    ...
    //Descontos
    float porc_inss[n], aux_saude[n], refeicao[n], transporte[n], falta[n], fgts[n], descontos[n], aux_saude_dep, soma;
    for (i=0; i<n; i++){
...
int main ()
{
    ...
    for (i=0; i<n; i++){
        fscanf (arq, " %[^\n] %f %f %d %c %c %d",lista[i].nome, &lista[i].sb, &lista[i].hr_ext, &lista[i].dias_falta, &lista[i].transporte, &lista[i].refeicao, &lista[i].dep);
        if (lista[i].dep>0){
            for (j=0; j<lista[i].dep; j++){
