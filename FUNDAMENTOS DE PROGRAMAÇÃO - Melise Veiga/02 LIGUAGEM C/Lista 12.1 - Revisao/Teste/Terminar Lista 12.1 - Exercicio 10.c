...
int main ()
{
    ...
    for (i=0; i<n; i++){
        fprintf (arq, "\n %s\n %s\n %s\n %c\n %d\n %d\n %d\n %d\n %d\n %.2f\n %.2f\n %d\n %d\n %c\n %c\n %d\n",lista[i].nome, lista[i].endereco, lista[i].civil, lista[i].sexo, lista[i].mat, lista[i].cpf, lista[i].rg, lista[i].titulo, lista[i].conta, lista[i].sb, lista[i].hr_ext, lista[i].nascimento, lista[i].dias_falta, lista[i].transporte, lista[i].refeicao, lista[i].dep);
        if (lista[i].dep>0){
