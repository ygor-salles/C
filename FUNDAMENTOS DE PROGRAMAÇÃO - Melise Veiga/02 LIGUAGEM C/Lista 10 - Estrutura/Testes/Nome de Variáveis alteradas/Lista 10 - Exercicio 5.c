...
void funcaoB (taluno list[], int n)
{
    int i;
    printf ("\nSeguido por: Nome / Matricula / Código / Rendimento\n");
    for (i=0; i<n; i++){
        printf ("\n %s %7d %10d %9.2f", list[i].nome, list[i].mat, list[i].cod, list[i].rend);
        printf ("\n");
    }
    return ;
}

void funcaoC (taluno list[], int n)
{
    int i, matricula, semafaro=0;
    printf ("\n\nDigite uma matrícula: "); scanf ("%d",&matricula);
    for (i=0; i<n; i++){
        if (list[i].mat == matricula){
...
void funcaoD (taluno list[], int n)
{
    int i;  char nome[51];
...
void funcaoE (taluno list[], int n, int *qtd, char nome[])
{
    int i;
    *qtd=0;
    printf ("\n\nDigite um nome: ");    scanf (" %[^\n]",nome);
