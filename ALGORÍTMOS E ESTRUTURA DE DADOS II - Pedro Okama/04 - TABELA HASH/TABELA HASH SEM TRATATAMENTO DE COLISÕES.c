...
int insere_hash (Tabela *hash, Registro reg)
{
    ...
    //ALOCAR UMA POSIÇÃO PARA RECEBER O REGISTRO
    Registro *novo;
    novo = (Registro*) malloc(sizeof(Registro));
    if (novo == NULL)
        return 0;

    //COPIAR OS DADOS PASSADOS PARA A POSIÇÃO DA TABELA
    *novo = reg;
    hash->itens[pos] = novo;
    hash->qtd++;
...
int consulta_hash (Tabela *hash, int chave, Registro *reg)
{
    ...
    //CALCULAR A CHAVE DA DIVISÃO
    int pos = chave_divisao(chave, hash->TABLE_SIZE);
    if (hash->itens[pos] == NULL) //VERIFICAR SE FOI ALOCADO NA POSIÇÃO DO VETOR
        return 0;

    //SE FOI ALOCADO, COPIAR A POSIÇÃO DO ARRAY PARA O PONTEIRO PASSADO POR REFERÊNCIA QUE RETORNARÁ O REGISTRO CORRETO
    *reg = *(hash->itens[pos]);
